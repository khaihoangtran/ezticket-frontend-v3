...
export default function MenuTab({ list }) {
	return (
		<Tabs
			theme={customTheme}
		...
		>
			{list.map((item, index) => {
				return (
					<Tabs.Item key={index} title={item.tab} icon={item.icon}>
						<section className="w-[100%] min-h-[90dvh] bg-gray-100 ">
							{item.content}
						</section>
...
const customTheme = {
	...
	tablist: {
		...
		styles: {
			...
			fullWidth:
				"w-full overflow-x-hidden text-sm font-medium divide-x divide-gray-200 shadow grid grid-flow-col dark:divide-gray-700 dark:text-gray-400 rounded-none",
		},
		tabitem: {
	...
	tabitemcontainer: {
		base: "bg-black-100 text-gray-900 mt-[-8px]",
		styles: {
			default: "",
			underline: "",
			pills: "",
